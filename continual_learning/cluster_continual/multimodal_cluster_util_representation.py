...
class Multimodal_utils():
    ...
    def Train(self,model, optimizer, loss_fn, train_loaders, vali_loader, early_stopping):
        ...
        times = [len(loader) for loader in train_loaders]
        times = max(times)
        # print(times)
        t = 0 
        while True:
            ...
            for idx, loader_iter in enumerate(loaders_iter):
                try:
                    ...
                    # 데이터 로더가 끝난 경우 다시 시작
                    loader_iter = iter(train_loaders[idx])
                    batch.append(next(loader_iter))
            # 각 데이터 로더에서 데이터 추출
        ...
        # grad_lst =np.array([grad.detach().cpu().numpy() for grad in grad_lst])
    ...
    # def replay_loader(self,replay,new):
